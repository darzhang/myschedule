...
export default function AddEventDialog({open, handleClose, onSubmit}) {
  ...
  const handleSubmit = async() => {
    ...
    const docRef = await addDoc(collection(db,process.env.NEXT_PUBLIC_FIREBASE_EVENT_COLLECTION), {
      title: event.title,
    ...
    await updateDoc(doc(db, process.env.NEXT_PUBLIC_FIREBASE_USER_COLLECTION, user.uid), {
  ...
  return (
    <Dialog open={open} onClose={closeDialog}>
      <Box sx={{m:"20px"}}>
        ...
        <Button onClick={async() => await handleSubmit()} variant='outlined'>Submit</Button>
      </Box>
    </Dialog>
  )
}