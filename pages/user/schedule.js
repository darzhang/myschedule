...
export default function SchedulePage() {
  ...
  useEffect(() => {
    const fetchEvents = async() => {
      ...
      const q = query(collection(db, process.env.NEXT_PUBLIC_FIREBASE_EVENT_COLLECTION), where(documentId(), "in", eventIds));
    ...
    const fetchEventIds = async() => {
      const userRef = doc(db, process.env.NEXT_PUBLIC_FIREBASE_USER_COLLECTION, user.uid);
